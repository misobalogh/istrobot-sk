<section>
    <h3 class="font-semibold">{{ __('dashboard_messages.set_year_title') }}:</h3>
    ...
    <form method="post" action="{{ route('admin.setYear') }}" class="mt-6 space-y-6">
        ...
        <div class="flex gap-4">
            ...
            {{ __('dashboard_messages.save') }}
            </x-primary-button>

            @if (session('status') === 'year-set-successfuly')
            <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)"
                class="text-sm text-gray-600 dark:text-gray-400">{{ __('dashboard_messages.saved') }}</p>
            @endif
        </div>
    </form>
</section>